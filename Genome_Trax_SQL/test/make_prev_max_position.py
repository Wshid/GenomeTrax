...
def make_max_file_sql(chromosome_range, hg_version):
    for chr_num in chromosome_range:
        ...
        ##sql="""SELECT MAX(feature_end) FROM genometrax_schema.ngs_feature_%s_%s"""
        sql="SELECT MAX(feature_end) FROM genometrax_schema.ngs_feature_%s_%s"%(hg_version, chr_string)
...
if __name__=="__main__":
    ...
    for hg_version in hg_versions:
        ...
        output_max_file=open("list_max_"+str(hg_version), "w")
...
def make_max_file(chromosome_range): ## chromosome에 따른 최대 길이를 반환한다.
    for chr_num in chromosome_range:
        ...
        sql="""SELECT feature_end FROM genometrax_schema.ngs_feature_%s_%s"""
        curs.execute(sql, (hg_version, chr_string))
